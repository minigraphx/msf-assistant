...
- Deploy exactly one application process (one Uvicorn worker and one container); request/rate/sync capacity limits are process-local. Multiple workers or replicas require a redesign. Cross-process player and maintenance locks remain required.
...
- Task 5 complete: hosted CLI, consistent backup/restore and pinned deployment bundle at 85666f3; independent review (no critical findings) addressed in the follow-up commit: systemd stop before restore, stale `.msf-*` writer leftovers skipped by backup, Claude "register automatically" hint, explicit ports rejected, friendly error without the hosted extra, CLI dispatch/ASGI restored-token tests. 300 tests and Ruff passed. Image built from the pinned digest with hash-checked dependencies; container smoke test passed (health/healthcheck, OAuth metadata, 401 for missing/forged bearer, one worker despite WEB_CONCURRENCY, ~81 MiB idle, no secrets in logs, state survives restart). Wheel 0.4.0 installs without extras and local CLI is unaffected.
- Task 6 step 1: whole-branch review of Tasks 1–5 (cross-task seams, spec audit, local-mode regression, secrets sweep, pin sanity). One critical finding fixed at d9873fe: the SDK's protected-resource metadata shadowed the provider's and advertised only `msf:read`. Local mode has no behavioural regression; no secret reaches logs/HTML/reprs; lock and deletion/restore seams verified. 301 tests, Ruff and CI (3.12–3.14) passed; PR #3 opened for merge into main. Remaining review items before public release: client-registration expiry after revoke/restore, hosted-specific error guidance instead of local CLI hints, nginx per-IP limits on unauthenticated OAuth endpoints, `hosted delete-player` command, exception-class logging, `WRITE_TOOLS` derived from tool annotations, consent page showing the connected account, wrong-`code_verifier` HTTP test.
- Task 6 deployment (branch `codex/hosted-deploy`): public name changed to `msf.andywhv.de` (wildcard A record and wildcard certificate already on the host); `deploy/install-host.sh` performs the idempotent host layout; the operator ran it and the service is live behind nginx (external checks: TLS, 308 redirect, HSTS, health, both metadata documents, 401 with scope challenge, 413 on oversized bodies, landing/privacy pages). MSF app registration, operator identity and real-client acceptance are still pending.
- Whole-branch review items addressed at 5df269f: PRM issuer identifier slash-free; client registrations survive revocation for the grant lifetime; hosted tool errors name the hosted login page and refresh_data instead of local CLI commands; German privacy notice and terms with configured operator identity (required by hosted config); `hosted delete-player`; route + exception-class logging; nginx per-IP limits and OAuth body caps; WRITE_TOOLS pinned to annotations; consent page names the connected account; wrong PKCE verifier HTTP test. 311 tests, Ruff and container smoke test passed; image `msf-assistant:0.4.0-5df269f` loaded on the host.
- MSF API Terms of Use review (user-supplied text, 3 March 2022 version): compliant on pricing, age, marketing/monetization, key confidentiality, no scraping, password handling, rate limits and de-link deletion; gaps fixed: 30-day snapshot TTL with deletion on access, snapshots excluded from backups and legacy archives restored without them, archive expiry by age, Scopely attribution on every page, "as is" statement in the privacy notice, operator duties (24 h breach notice, termination clean-up) in the ops guide. Public name and title renamed to `advisor.andywhv.de` / "Strike Advisor" because §2k forbids Scopely or licensor marks in the Application title or URL; internal paths keep `msf-assistant`.
- Repository made self-hostable for other operators: configurable `MSF_SERVICE_NAME`, hostname/name mark refusal, nginx template rendered by a generic `install-host.sh` (`PUBLIC_URL`, `CERT_NAME`, `NGINX_REPLACE`), operator-specific docs removed or genericized, English README, copyright line added. Git history scanned: no secrets, no roster data.
